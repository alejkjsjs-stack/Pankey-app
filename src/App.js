...
const injectFirebase = () => {
  ...
  s.innerHTML = `
    ...
    try {
      ...
      window.__FB = { db: getDatabase(app), ref, set, get, onValue, update, push, onDisconnect, auth, provider, signInWithPopup, signInWithRedirect, getRedirectResult };
...
const seenNotifsRef = useRef(new Set()); // Para no spamear al usuario con la misma noti
  ...
  useEffect(() => {
    ...
    const checkFb = setInterval(async () => {
      if (!window.__FB_READY || !window.__FB) return;
      ...
      try {
        ...
        if (snapUid.exists()) {
          ...
          if (userSnap.exists()) {
            ...
            setScreen(basicUser.partnerCode || basicUser.sessionId ? 'app' : 'connect');
          }
        } else {
          window.__FB_REDIRECT_USER = { uid, googleName: res.user.displayName || '' };
          setScreen('onboarding');
        }
      } catch(e) { /* no redirect pendiente */ }
    }, 100);
...
function Onboarding({ C, isLight, onDone }) {
  ...
  useEffect(() => {
    if (window.__FB_REDIRECT_USER) {
      const { uid, googleName } = window.__FB_REDIRECT_USER;
      window.__FB_REDIRECT_USER = null;
      setTempUid(uid); setName(googleName); setStep('profile');
    }
  }, []);

  const handleGoogleLogin = () => {
    if (!fbOK()) { setError('Firebase no está listo. Espera un momento.'); return; }
